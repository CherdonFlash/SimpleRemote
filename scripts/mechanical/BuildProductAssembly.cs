...
internal static class BuildProductAssembly
{
    ...
    private static void SaveView(ModelDoc2 model, string viewName, int viewId, string path)
    {
        ...
        model.SetUserPreferenceToggle((int)swUserPreferenceToggle_e.swDisplayPlanes, false);
        model.ViewDisplayShaded();
        model.ShowNamedView2(viewName, viewId);
        model.ViewZoomtofit2();
        Save(model, path);
        // 通过SolidWorks渲染API导出高分辨率位图，不捕获桌面或操作窗口。
        string bitmapPath = Path.Combine(Path.GetTempPath(), "SimpleRemote-render-" + Guid.NewGuid().ToString("N") + ".bmp");
        if (model.SaveBMP(bitmapPath, 2400, 1600))
    ...
    public static int Main(string[] args)
    {
        if (args.Length < 4)
        {
            Console.Error.WriteLine("Usage: BuildProductAssembly <obj> <lower-part> <upper-part> <output-dir>");
            return 2;
        }
        ...
        string pcbPart = Path.Combine(outputDir, Path.GetFileNameWithoutExtension(objPath) + ".SLDPRT");
        string pcbPreviewPath = Path.Combine(outputDir, "PCB_预览.png");
        string pcbPreviewZoomPath = Path.Combine(outputDir, "PCB_预览放大.png");
        string assemblyPath = Path.Combine(outputDir, "遥控器产品装配体.SLDASM");
        string previewPath = Path.Combine(outputDir, "产品装配图.png");
        string frontViewPath = Path.Combine(outputDir, "产品装配体_俯视图.png");
        string typeCViewPath = Path.Combine(outputDir, "产品装配体_TypeC侧视图.png");
        string switchViewPath = Path.Combine(outputDir, "产品装配体_电源开关侧视图.png");
        string nrfViewPath = Path.Combine(outputDir, "产品装配体_NRF24侧视图.png");
        ...
        try
        {
            ...
            // OBJ 坐标映射到外壳基准；板框匹配后使用已核对的平移量。
            // 支柱顶面16 mm + PCB厚1.6 mm = 板顶17.6 mm。
            AddFixed(sw, assemblyModel, assembly, pcbPart, -21.122147, -41.820067, 17.6);
            ...
            Save(assemblyModel, assemblyPath);
            Save(assemblyModel, Path.Combine(outputDir, "遥控器新PCB校核装配体.SLDASM"));
            return 0;
        }
        catch (Exception ex)
        {
