...
SimpleRemote 是一套基于 STM32F103 的简易无线遥控器工程。遥控器采集摇杆、按键和拨动开关，通过 CW2015CHBD 获取锂电池电压及剩余电量，在 OLED 上显示当前状态，并通过 NRF24L01 将控制数据发送给接收端。
...
flowchart TB
    ...
    subgraph BAT["电池与电量监测"]
        ...
        B1["CW2015CHBD 电量计<br/>电池电压 · SOC · RRT<br/>I2C2（PB10/PB11）"]
        B0 --> B1
    end

    subgraph MCU["STM32F103C6 主控<br/>裸机主循环 · 72 MHz"]
        P1["数据采集 / 映射<br/>按键状态机 · CW2015数据读取"]
    end
...
- 读取 1 个双向拨动开关状态。
- 通过 CW2015CHBD 读取锂电池电压、SOC和剩余运行时间信息。
...
| CW2015CHBD | 单节锂电池电量计，通过独立I2C2提供电压、SOC和RRT信息 |
...
| CW2015电量计I2C2 | PB10/SCL、PB11/SDA |
...
完成CW2015固件适配后，上电程序将依次初始化72 MHz系统时钟、SysTick、独立看门狗、串口、按键定时器、摇杆ADC DMA、OLED、I2C2电量计和NRF24L01。主循环持续执行以下操作：

1. 将 ADC 数据转换为摇杆控制量并更新 OLED 显示缓冲区。
2. 通过I2C2读取CW2015的电池电压、SOC和RRT寄存器。
3. 组合摇杆、按键、拨动开关和电池信息。
...
> 新版遥控器V1.1硬件已经使用CW2015CHBD，并通过PB10/PB11的I2C2读取电池信息；当前固件仍保留旧版PB1 ADC逻辑，上述V1.1软件流程尚待实现。
...
电子设计源文件位于 `hardware/electronics/`。当前投板版本统一命名为“新版遥控器V1.1”，包含 `新版遥控器V1.1.epro`、`新版遥控器V1.1.tel` 和 `新版遥控器V1.1.pdf`，旧版设计导出文件已删除。V1.1包含91个器件位号、61个网络；静态复核确认网表中的279项明确引脚连接与PCB焊盘网络完全一致。详细设计条件和投板检查项见该目录的 `README.md` 与 `REVIEW_新版遥控器V1.1.md`。
