...
// V1.1 只采集 PA2、PA3、PA4、PB0 四路摇杆，电池数据由 CW2015 提供。
void mobile_init(){
    ...
    // PB0为第四路摇杆；PB1不再采样，仅保持模拟输入以关闭悬空数字输入缓冲。
    ...
    DMA_InitStructure.DMA_BufferSize = ADC_CHANNELS; // 4通道
    ...
    ADC_InitStructure.ADC_NbrOfChannel = ADC_CHANNELS;   // 4通道
